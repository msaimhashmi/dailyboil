...
function test_connection(){
    ...
    $result = curl_exec($ch);
    $hasCaptcha = curl_errno($ch) == 0 && (curl_getinfo($ch, CURLINFO_HTTP_CODE) == 429 || preg_match('/recaptcha/i', $result) == 1);
    curl_close($ch);
    return $hasCaptcha;
}
...
{
    $cookie_file = __DIR__ . "/storage/temp/cookies-" . $_SESSION["token"] . ".txt";
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_HEADER, 0);
...
{
    $fsize = get_file_size($remoteURL, false);
    header('Content-Description: File Transfer');
    header('Content-Type: application/octet-stream');
    header('Content-Disposition: attachment; filename="' . sanitize_filename($vidName, $ftype) . '"');
